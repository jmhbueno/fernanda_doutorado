...
filmes_ordenados <- arrange(imdb, ano, desc(receita))  #notar que para ascendente, não precisa colocar nada, so a variável (ver ano).
...
resposta_do_exercício <- arrange(titorc, desc(orcamento))
...
imdb %>% filter(!diretor %in% c("Quentin Tarantino", "Steven Spielberg"))
# na frase acima, a exclamação serve para negar, todos menos Tarantino e Spielberg.
...
curtos_legais <- filter (imdb, duracao< 90 & nota_imdb > 8.5) # notação normal.

curtos_legais <- imdb %>% filter (duracao < 90 & nota_imdb > 8.5) # notação usando PIPE.

# diferença entre filter e select.
# filter é para selecionar linhas... 
# select é para selecionar colunas
...
# A fun��o ifelse � uma �tima ferramenta
# para fazermos classifica��o bin�ria
...
imdb %>% group_by(cor) # a partir do uso desse comando, as analises subsequntes serão por cor.
imdb %>% ungroup(cor)
